...
# 2. Graphiques pour les nuits (23h00 - 3h00)
df_night = df_first_7_days_hourly[df_first_7_days_hourly["DateTime_x"].dt.hour.isin([23, 0, 1, 2, 3])]

fig, axs = plt.subplots(5, 1, figsize=(12, 12), sharex=True)
fig.suptitle("Analyse des nuits (23h00 - 3h00)", fontsize=16)

# Ajout du fond gris pour les fins de semaine
for i in range(len(df_night) - 1):
    start = df_night["DateTime_x"].iloc[i]
    end = df_night["DateTime_x"].iloc[i + 1]
    if start.weekday() >= 5:  # 5 = Samedi, 6 = Dimanche
        for ax in axs:
            ax.axvspan(start, end, color="lightgray", alpha=0.5)

# Tracer le son (Leq_dBA)
axs[0].plot(df_night["DateTime_x"], df_night["Leq_mean"], linestyle="-", color="b", label="Leq (dBA)")
axs[0].fill_between(df_night["DateTime_x"], df_night["Leq_min"], df_night["Leq_max"], color="b", alpha=0.2)
axs[0].set_ylabel("Niveau sonore (dBA)")
axs[0].legend()
axs[0].grid(True)

# Tracer la lumière (lux_S1) en échelle logarithmique
axs[1].plot(df_night["DateTime_x"], df_night["Lux_mean"], linestyle="-", color="orange", label="Lumière (lux)")
axs[1].set_yscale("log")  # Échelle logarithmique
axs[1].set_ylabel("Éclairement (lux)")
axs[1].legend()
axs[1].grid(True, which="both")

# Tracer la température (°C)
axs[2].plot(df_night["DateTime_x"], df_night["Temp_mean"], linestyle="-", color="red", label="Température (°C)")
axs[2].set_ylabel("Température (°C)")
axs[2].legend()
axs[2].grid(True)
...
# Tracer l'humidité relative (%)
ax4 = axs[3]
ax4.plot(df_night["DateTime_x"], df_night["Humidity_mean"], linestyle="-", color="green", label="Humidité (%)")
ax4.set_ylabel("Humidité (%)")
ax4.legend(loc="upper left")
ax4.grid(True)
...
# Ajouter un axe secondaire pour les précipitations
ax5 = ax4.twinx()
ax5.plot(df_night["DateTime_x"], df_night["precipitations"], linestyle="-", color="cyan", label="Précipitations (mm)")
ax5.set_ylabel("Précipitations (mm)")
ax5.legend(loc="upper right")

# Tracer la vitesse du vent (km/h) et la pression atmosphérique (kPa)
ax6 = axs[4]  # Dernier subplot
ax6.plot(df_night["DateTime_x"], df_night["Wind_mean"], linestyle="-", color="purple", label="Vent (km/h)")
ax6.set_ylabel("Vent (km/h)")
ax6.legend(loc="upper left")
ax6.grid(True)
...
# Ajouter la pression atmosphérique sur le même axe mais avec une deuxième échelle Y
ax7 = ax6.twinx()
ax7.plot(df_night["DateTime_x"], df_night["Pressure_mean"], linestyle="-", color="brown", label="Pression (kPa)")
ax7.set_ylabel("Pression (kPa)")
ax7.legend(loc="upper right")
...
# 3. Graphiques pour chaque mois (novembre, décembre, janvier)
